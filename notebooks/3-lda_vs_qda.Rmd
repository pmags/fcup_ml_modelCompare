...
```
From the above plots we can conclude that the results are in line with our initial hypothesis. LDA models shows better results with the the LDA dataset, but not so different of QDA. While, it is strong the power of QDA model in the QDA dataset, because the different covariance between the classes.

The following plot show the metrics of each model calculated on each of the 10 validation folds. 
